...
function renderLoopInterviewTab(el, co) {
  ...
  html += '.loop-nav-dropdown{position:relative;padding-bottom:10px;margin-bottom:-10px}'; // invisible bottom bridge
  html += '.loop-nav-menu{display:none;position:absolute;top:100%;left:-8px;background:var(--color-bg-card);border:1px solid var(--color-border);border-radius:var(--radius-sm);padding:10px 8px;min-width:200px;box-shadow:0 6px 16px rgba(0,0,0,0.4);z-index:200}';
  html += '.loop-nav-dropdown:hover .loop-nav-menu,.loop-nav-menu:hover{display:block}';
  html += '.loop-nav-submenu{position:relative;padding:10px 14px;cursor:pointer;font-size:0.8rem;color:var(--color-text);display:flex;justify-content:space-between;align-items:center;border-radius:4px;font-weight:600;margin-right:-8px;padding-right:22px}'; // padding-right extends hover area toward submenu
