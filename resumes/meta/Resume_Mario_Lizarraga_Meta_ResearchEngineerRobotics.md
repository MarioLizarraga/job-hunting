MARIO LIZARRAGA
Redmond, WA | mario.lizarragat@gmail.com | (425) 543-7328 | linkedin.com/in/mario-lizarraga

SUMMARY

Mechatronics engineer with 7+ years designing electromechanical systems, writing embedded firmware, and building robotic prototypes in aerospace. Took a robotic test platform from whiteboard to production — cut cycle time by 67% and freed 1,300+ hrs/year. MSc in Space Systems Engineering (Mars Rover and CubeSat mechanisms).

EXPERIENCE

AUTOMATION ENGINEER II | Feb 2023 – Present
Safran Electronics & Defense — Redmond, WA

- Designed and built a robotic test platform — UFactory 850 arm, FUTEK load cell, 3D-printed fixtures, STM32 continuity tester — cutting test time by 67% across 75 panels/week
- Wrote STM32 firmware in C for a custom continuity board — serial comms, signal acquisition, debounce logic for real-time validation during robot press cycles
- Modeled and iterated test fixtures and end-effectors in SolidWorks — sub-millimeter alignment through 4+ revisions (FDM/SLA)
- Developed a multi-axis gantry (X, Y, Z, rotation) for automated panel inspection — mechanical layout, motion control, DSLR integration
- Supervised 5 interns on AI defect detection and robotics — delivered a 92% accuracy ML model and hardware prototype for vision-based inspection

DESIGN ENGINEER | Nov 2021 – Dec 2022
Jenton International — Whitchurch, UK

- Designed custom automation machinery in Autodesk Inventor — mechanical layouts, pneumatics, electromechanical assemblies
- Built and commissioned equipment on the shop floor, diagnosing mechanical, electrical, and sensor issues

DESIGN ENGINEER | Jun 2019 – Sep 2020
Honeywell Aerospace — Mexicali, Mexico

- Designed jet engine APU components in CATIA V5, applying GD&T per ASME Y14.5 and tolerance stackup analysis
- Completed Six Sigma Green Belt DFSS — applied DOE and statistical methods to tolerance analysis

AR/VR PROJECT ENGINEER | Jul 2017 – Oct 2019
El Garage Project Hub — Mexicali, Mexico

- Led the NASA Space Apps Challenge (175+ participants); built AR/VR prototypes in Unity (C#) and taught robotics workshops to 50+ students

EDUCATION

MSc Space Systems Engineering (Merit) — University of Southampton, UK | 2020 – 2021
BSc Mechatronics Engineering (Magna Cum Laude, 96.36) — CETYS Universidad, Mexico | 2013 – 2017

SKILLS

Design & Prototyping: fixture/end-effector design, mechanism design, 3D printing (FDM/SLA), CNC awareness, sheet metal
Electromechanical: robot arm integration (xArm SDK), force sensors (FUTEK), vibration measurement, I2C/SPI/UART
Technical: STM32 (C), Python, SciPy, OpenCV, SQL, Git, SolidWorks, CATIA V5, Autodesk Inventor, Arduino, Raspberry Pi
Methods: GD&T, tolerance analysis, DFM/DFA, Six Sigma DFSS, machine learning, computer vision

CERTIFICATIONS: CATIA V5 Expert — Dassault | Six Sigma Green Belt DFSS — Honeywell | GD&T — Applied Geometrics
LANGUAGES: English (fluent) | Spanish (native)
